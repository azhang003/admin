...
class capcha
{
    ...
    protected function generate(): array
    {
        ...
        if ($this->math) {
            ...
            CapchaCache::set($key, $and, $this->expire);
        } else {
            if ($this->useZh) {
            ...
            CapchaCache::set($key, $val, $this->expire);
        }
    ...
    public function check($verify_id, $verify_code): bool
    {
        ...
        $verify = CapchaCache::get($verify_id);
        CapchaCache::del($verify_id);
    ...
    public function create(string $config = null, bool $api = false)
    {
        ...
        foreach ($text as $index => $char) {
            ...
            $x     = $this->fontSize * ($index + 1) * mt_rand(1.01, 1.2) * ($this->math ? 1 : 1.2);
            $y     = $this->fontSize + mt_rand(10, 20);
            $angle = $this->math ? 0 : mt_rand(-40, 40);
            putenv('GDFONTPATH=' . realpath('.'));
            imagettftext($this->im, $this->fontSize, $angle, $x, $y, $this->color, __DIR__.$ttfPath.$this->fontttf, $char);
        }
        ob_start();
        // 输出图像

...
class MemberDate extends Command
{
    protected function configure(){
        $this->setName('memberdate')->setDescription("计划任务 用户每日数据统计");
    }
...
//        $time = strtotime(date("Y-m-d",time()-86400))+7*3600;
        ...
        $Balance['mid'] = $uid;
        $Balance['day_bet_count'] = MemberRecord::where([['mid','=',$uid],['business','=',3],['currency','=',1],['create_time','>',$time]])->count();
        $Balance['day_bet'] = abs(MemberRecord::where([['mid','=',$uid],['business','=',3],['currency','=',1],['create_time','>',$time]])->sum('now'));
        $Balance['day_profit'] = abs(MemberRecord::where([['mid','=',$uid],['business','=',4],['currency','=',1],['create_time','>',$time]])->sum('now'))-$Balance['day_bet'];
